...
# )
def BLOCK_CONFIRMATIONS_FOR_VERIFICATION():
    return 1 if network.show_active() in LOCAL_BLOCKCHAIN_ENVIRONMENTS else 6
...
GAS_PRICE_LINK = 10**9  # Some value calculated depending on the Layer 1 cost and Link
...
def listen_for_event(brownie_contract, event, timeout=200, poll_interval=2):
    ...
    try:
        ...
        return {"event": "Keyboard Interrupted! Not found any events!"}
    return {"event": None}
